...
if __name__ == "__main__":
    ...
    airport_codes_df = spark.read.option("header", True).option("inferSchema", True)\
                            .csv('s3a://landing-layer-udacity-nd//airport-codes_csv.csv')
    print(airport_codes_df.printSchema())
    airport_codes_df.write.format("delta").mode("overwrite").save("s3a://bronze-layer-udacity-nd/airport_codes")
