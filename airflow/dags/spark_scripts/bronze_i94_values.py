...
def generate_spark_dataframe_from_file(
    spark, file_path: str, header_identifier: str
) -> DataFrame:
    """Generate spark dataframe extrating some content related to the
    header_identifier of a file
    ...
    for line in searched_values.split("\n"):
        if line.find("=") != -1:
            ...
            values.append(
                [breaked_line[0].strip(" '\t"), breaked_line[1].strip(" '\t")]
            )
...
if __name__ == "__main__":
    ...
    I94cit_res_df = generate_spark_dataframe_from_file(
        spark,
        "s3a://landing-layer-udacity-nd/I94_SAS_Labels_Descriptions.SAS",
        "I94CIT",
    )
    print(I94cit_res_df.printSchema())
    I94cit_res_df.write.format("delta").mode("overwrite").save(
    ...
    I94port_df.write.format("delta").mode("overwrite").save(
    ...
    I94mode_df.write.format("delta").mode("overwrite").save(
    ...
    I94addr_df.write.format("delta").mode("overwrite").save(
    ...
    I94visa_df.write.format("delta").mode("overwrite").save(
